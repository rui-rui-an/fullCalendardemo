<template>
  <div>
    <DatePicker :value="dateRange" @on-change="dateChange" format="yyyy-MM-dd" placeholder="Select date" type="daterange" style="width: 200px"></DatePicker>
    <br />
...
export default {
  ...
  data() {
    return {
      ...
      calendarApi: null,
      calendarOptions: {
        plugins: [dayGridPlugin, interactionPlugin, timeGridPlugin, multiMonthPlugin], // 需要用哪个插件引入后放到这个数组里
        initialDate: new Date(), // 日历第一次加载时显示的初始日期。可以解析为Date的任何职包括ISO8601日期字符串，例如"2014-02-01"。
        initialView: 'dayGridFourWeek', // 日历加载时的初始视图，默认值为'dayGridMonth'，可以为任何可用视图的值，如例如'dayGridWeek'，'timeGridDay'，'listWeek'
  ...
  methods: {
    ...
    setCalender() {
      setTimeout(() => {
        ...
        // 发送请求 请求数据
        const exMonth = dayjs(this.dateRange[1]).format('MM') - dayjs(this.dateRange[0]).format('MM') + 1;
        this.calendarOptions.validRange.start = this.dateRange[0];
        console.log('start', this.calendarOptions.validRange.start);
        this.calendarOptions.validRange.end = this.dateRange[1];
        console.log('end', this.calendarOptions.validRange.end);
        this.calendarOptions.views.dayGridFourWeek.duration.months = exMonth;
        // console.log('duration.months', this.calendarOptions.views.dayGridFourWeek.duration.months);
        this.$nextTick(() => {
          this.calendarApi.gotoDate(this.dateRange[0]);
        });
        //
      }, 1000);
    },
